...
Существует сервис, который позволяет верифицировать, что на определенном адресе в TON выполняется определенный смарт-контракт. Для этого вам нужно иметь доступ к файлу с исходным кодом. Давайте посмотрим как это работает на примере TON Fruits:
